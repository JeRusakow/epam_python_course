...
from homework12.models import DeadlineError, Homework, HomeworkResult, Student, Teacher

date_01_jan_2020 = datetime.datetime(year=2020, month=1, day=1)

date_01_jan_2021 = datetime.datetime(year=2021, month=1, day=1)

test_student = Student("John", "Doe")

test_teacher = Teacher("Janne", "d'Arc")

test_homework = Homework("Some task", 10, test_teacher)
...
def test_teacher_generates_proper_homeworks():
    hw_text = "Some task"
    days_to_complete = 5
    proper_deadline = date_01_jan_2020 + datetime.timedelta(days=days_to_complete)

    with unittest.mock.patch("datetime.datetime") as fake_datetime:
        fake_datetime.now.return_value = date_01_jan_2020
        homework_from_teacher = test_teacher.create_homework(hw_text, days_to_complete)
...
def test_check_homework_returns_false_on_poor_homework():
    ...
    hw = Homework("Some task", 10, teacher)
...
def test_homeworks_added_to_homework_done():
    with unittest.mock.patch("datetime.datetime") as fake_datetime:
        ...
        hw = Homework("Some other task", 10, teacher)
